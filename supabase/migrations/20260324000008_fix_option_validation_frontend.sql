-- Fix: create_order_atomic option validation supports frontend format (group/choice names)
-- Previous version only accepted option_choice_id, rejecting frontend orders
...
BEGIN
  ...
  -- ========== Rate limit: max 5 orders per table per minute ==========
  ...
  -- ========== Option validation (supports both formats) ==========
  -- Format 1 (API): { option_choice_id: uuid, ... }
  -- Format 2 (Frontend): { group: "name", choice: "name", extra_price: 0 }
  FOR v_item IN
    SELECT
      item.menu_item_id,
  ...
  LOOP
    ...
    LOOP
      ...
      -- Strategy 1: validate by option_choice_id
      ...
      -- Strategy 2: validate by group name + choice name
      ...
      -- If neither format matched a valid option, skip (don't block order)
      -- The enforce_menu_item_price trigger will handle price correction
    END LOOP;
  END LOOP;
