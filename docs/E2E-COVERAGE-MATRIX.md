...
### P0 미커버 — ✅ 모두 해결 (2026-03-24)

| UC-ID | 설명 | 상태 | 테스트 |
|-------|------|------|--------|
| UC-C07~C08 | 옵션 선택 모달 진입/UI | ✅ 해결 | `order-detail.spec.ts` UC-C07~C08 |
| UC-C12~C13 | 아이템 수량 설정 + 장바구니 추가 | ✅ 해결 | `order-detail.spec.ts` UC-C12~C13 |
| UC-SA08~SA09 | 기간 연장/강제 정지 해제 후 복구 | ✅ 해결 | `edge-cases.spec.ts` UC-SA08, UC-SA09 |

### 교차 검증에서 발견된 매핑 오류

| UC-ID | 매트릭스 기재 | 실제 | 수정 |
|-------|------------|------|------|
| UC-S22 (NT-002~005) | 커버됨 | ❌ stub 테스트 (test.skip) | 미커버로 재분류 |
| UC-S08 (SB-004) | 커버됨 | ❌ stub (expect(true)) | 미커버로 재분류 |
| SEC-E22 | staff 차단 | ⚠️ 실제로는 manager 테스트 | 라벨 수정 필요 |
| SEC-E15 | 서버 검증 | ⚠️ pre/post 모두 수락 | 약한 assertion |
